    ...
    if msgs > max_msg then
	  ...
	  if msg.to.type == 'chat' or msg.to.type == 'channel' then
		if username then
			...
			send_large_msg(receiver , '🎖<code> کاربر </code> (<b>'..msg.from.first_name..'</b>) <code> با شناسه کاربری </code> [<b> '..msg.from.id..' </b>] <code> به دلیل ارسال پیام بیش از حد از گروه اخراج شد </code>🎖')
      ...
      if gbanspamonredis then
        if tonumber(gbanspamonredis) ==  4 and not is_owner(msg) then
          ...
          send_large_msg("chat#id"..msg.to.id, "🎖کاربر [ "..name.." ]"..msg.from.id.." ازتمامی گروه های ژوپیتر محروم شد(Globally Banned!)🎖")
		  send_large_msg("channel#id"..msg.to.id, "🎖کاربر [ "..name.." ] با شناسه کاربری ["..msg.from.id.."] از تمامی گروه های ربات مسدود شد(Globally Banned!)🎖")
